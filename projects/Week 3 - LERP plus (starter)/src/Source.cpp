...
int main()
{
	...
	App::Init("Week 3 tutorial - LERP plus paths", 1600, 1200);
	...
	float test1 = 0.0f, test2 = 0.0f, test3 = 0.0f;
	...
	while (!App::IsClosing() && !Input::GetKeyDown(GLFW_KEY_ESCAPE))
	{
		...
		/*
		Entity ent_mesh_box = Entity::Create();
		ent_mesh_box.Add<CMeshRenderer>(ent_mesh_box, *mesh_box, *mat_unselected);
		ent_mesh_box.transform.m_scale = glm::vec3(0.1f, 0.1f, 0.1f);
		*/
